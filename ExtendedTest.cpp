...
void testLastIndexOf(){
    ...
    // tail case
    ListIterator it2 = list.lastIndexOf(10);
    assert(it2.getCurrent() == 10);
    it2.next(); // should be nullptr now
    try {
        it2.next();
        assert(false);
    }
    ...
    // head case
    ListIterator it3 = list.lastIndexOf(0);
    assert(it3.getCurrent() == 0);
    it3.next();
    assert(it3.getCurrent() == 1);
    // no element case
    ListIterator it4 = list.lastIndexOf(100);
    assert(!it4.valid());
}
